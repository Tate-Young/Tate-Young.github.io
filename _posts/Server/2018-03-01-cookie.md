...
background-image: https://static.oschina.net/uploads/space/2015/0406/201833_rQQV_120166.png
tags:
- http
---
# {{ page.title }}

**会话(Session)**跟踪是 Web 程序中常用的技术，用来跟踪用户的整个会话。理论上，一个用户的所有请求操作都应该属于同一个会话，而另一个用户的所有请求操作则应该属于另一个会话。常用的会话跟踪技术是 cookie 与 session。
...
session 也可设置过期时间，禁用 cookie 并不一定无法使用 session。除了 cookies ,客户端还可以将发送给服务器的数据包含在请求的 URL 中:
