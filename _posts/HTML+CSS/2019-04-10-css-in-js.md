...
[**CSS Modules**](https://github.com/css-modules/css-modules) 与 webpack 可以很好地搭配使用，我们可以自定义生成样式的命名规则:

```JS
// webpack.config.js
// modules 即为启用，localIdentName 是设置生成样式的命名规则
// [name]表示标签名，[local]表示类名，[hash:base64:5] 是按照给定算法生成的序列码
...
> `mode: open` 可以在开发工具中检查，并通过查询、配置任何公开的 CSS 属性或监听它抛出的事件来交互

我们可以看到 `<div>` 包含 `<b>` 并且 `<b>` 标签也渲染在了页面上，但是常规的 DOM 方法却找不到它，只能内部通过 `this.shadowRoot.querySelector` 访问:
...
要将 HTML 添加到影子根，你可以为其 innerHTML 属性分配 HTML 字符串或使用 `<template>` 元素:
...
当然官方还是更推荐一些基于 class 的策略，比如上面提到的 [**CSS Modules**](https://vue-loader-v14.vuejs.org/zh-cn/features/css-modules.html)，它是一个用于模块化和组合 CSS 的流行系统。**vue-loader** 提供了与 CSS 模块的一流集成，可以作为模拟 CSS 作用域的替代方案:
...
Angular 采用了视图封装的类 **ViewEncapsulation**，可以有以下三种值:
...
![ViewEncapsulation-None.png](https://i.loli.net/2019/04/13/5cb1c54c0d6aa.png)
