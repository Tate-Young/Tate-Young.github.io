...
update: 2021-11-08 10:30:00 GMT+0800 (CST)
description: add SSO description
...
**单点登录(Single Sign On，即 SSO)**是多域名企业站点流行的登录方式，通过上述的 OAuth 2.0 即可实现。指用户只需输入一次账密，在一处完成登录，之后可以直接进入所有业务系统。想要完成单点登录的效果，必须有一个唯一身份源，其他业务系统必须配合完成改造和对接。

**单点登出**指用户点击一次登出就能退出他登录的所有系统。
